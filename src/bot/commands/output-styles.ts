...
export async function execute(
  ...
  if (!available.includes(style)) {
    await interaction.editReply({
      content: L(
        `Style \`${style}\` not found. Available: ${available.map((s) => `\`${s}\``).join(", ") || "(none)"}`,
        `스타일 \`${style}\`을(를) 찾을 수 없습니다. 사용 가능: ${available.map((s) => `\`${s}\``).join(", ") || "(없음)"}`,
      ),
    });
    return;
  }
    ...
    embeds: [
      {
        ...
        description: L(
          "Style updated. Takes effect from the next message (current conversation is kept).",
          "스타일이 업데이트되었습니다. 다음 메시지부터 적용됩니다 (현재 대화 유지).",
        ),
        color: 0x5865f2,
      },
    ],
