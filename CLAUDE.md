...
- **Transient run-end retry**（`session-manager.ts` `isTransientRunFailure` + `TransientRunError`）：`run.wait()` 回 `status:"error"` 那條公開型別只有 `result?: string`，**沒有 error code / cause**（spike 用 `Object.getOwnPropertyNames` + `inspect({showHidden:true})` 證實 SDK 沒藏 metadata）。Production 觀察到 transient server-side failure 表徵：`result: undefined` + `durationMs < 5_000ms`（沒進到 tool 執行階段）。Heuristic：符合這兩個條件就 throw `TransientRunError`，外層 catch 的 `else if (error instanceof TransientRunError && !placeholder.cancelRequested)` 分支跑同樣的 cleanup（`agent?.close()` 同步 close、reset UI state、`Agent.resume(placeholder.agentId ?? resumeAgentId)`）+ `runAttempt` 重跑一次。retry 進入點是 markDone() 之後，所以多一步 `sessionDone = false` 讓 renderStatus 重新 active。fatal `status:"error"`（有 result text 或 durationMs ≥ 5_000ms）維持原本「直接送 ❌ + offline」。retry 失敗（再次 transient 或拋其他錯）就 fall-through 到 finalError handler。閾值（`TRANSIENT_RUN_MAX_DURATION_MS = 5_000`）是 spike 觀察到 2_994ms + 一些 buffer，不是 SDK 保證、是 heuristic — 要調就改這個常數。
