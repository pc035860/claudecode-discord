...
- **Cursor SDK long-lived client 壞 state**：bot process 跑久（觀察到 ~2 天）後，`@cursor/sdk` 內部的 `@connectrpc/connect` client 會累積壞 state，所有 `agent.send()` / `run.stream()` 都拋 `ConnectError: [unauthenticated]`（gRPC code 16），fresh process 跑 spike 完全正常。SDK 沒公開 API 重建 internal client → 只能重啟整個 process。`maybeSelfRestart()`（`src/utils/self-heal.ts`，共用模組）偵測到 ConnectError code 16（`Code.Unauthenticated`）會 **self-exit(1)** 讓 PM2（`autorestart: true`）拉起乾淨 process，受 env `AUTO_RESTART_ON_AUTH_ERROR` 控制、預設開。兩條路徑都會呼叫它：`unhandledRejection` handler (`src/index.ts`，code 16 逃成 unhandled stream reject) 和 `sendMessage` catch block (`session-manager.ts`，code 16 同步 throw 進 await)。守門：`process.uptime() < 300s` 的 code 16 只 log 不 exit（防 API key 真壞時 crash loop）、module-scope `restartScheduled` 一次性旗標（跨兩條路徑共享，不重複排程）、延遲 2s 讓 in-flight Discord 訊息 flush。觸發那則訊息仍會失敗，重啟後下一則才正常。`sendMessage` finally drain queue 前先查 `isRestartScheduled()`：restart pending 時清掉 in-memory queue（撐不過重啟）並通知使用者重送，不在壞連線上起新 run。`run.wait()` 回 `status:"error"` 那條沒有 error code，不走 self-heal。`parseApiError` 對 ConnectError 走獨立 branch；catch block 把訊息送 Discord 前**先檢查開頭是不是 `❌/⚠️`** 避免 double emoji。
